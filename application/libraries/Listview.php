...
class Listview {		
    ...
    function filter($table, $limit, $offset, $where)
	{
		lvw_db_filter();
		
		$data['query'] 		= $this->ci->db->select("SQL_CALC_FOUND_ROWS *", false)->from($table)->limit($limit, $offset)->where(array('company' => $where))->get();		
